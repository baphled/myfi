class User
  ...
  def monthly_income
    income_entries.empty? ? 0.0 : income_entries.this_month.collect(&:amount).map(&:to_f).inject(&:+)
  end

  def monthly_outgoing
    outgoing_entries.empty? ? 0.0 : outgoing_entries.this_month.collect(&:amount).map(&:to_f).inject(&:+)
  end
